...
# testing with user / tescan and with RelWithDebInfo (builds on win 10, fails on win 11)
...
class QtConan(ConanFile):
    ...
    options = {
        ...
        "disabled_features": [None, "ANY"],
         ...
         # Modules explicitly required for WebEngine
    ...
    # for now keep False since it prevents build fail due long relative paths (c1xx: fatal error C1083: Cannot open source file: )
    ...
    default_options.update({
        ...
        "qtpositioning": True,
        "qtquick3d": True,
        "qtquick3dphysics": True,
        "qtquickcontrols2": True,
        "qtquickeffectmaker": True,
        "qtquicktimeline": True,
        "qtscxml": True,
        "qtsensors": True,
        "qtspeech": True,
        "qtsvg": True,
    ...
    def requirements(self):
        # tescan needs this with tescan/stable
        self.requires("zlib/[>=1.2.11 <2]@tescan/stable")

        if self.options.openssl:
            # tescan needs this with tescan/stable
            self.requires("openssl/[>=1.1 <4]@tescan/stable")
        if self.options.with_pcre2:
            # tescan needs this with tescan/stable
            self.requires("pcre2/10.42@tescan/stable")
        ...
        if self.options.with_glib:
            # tescan needs this with tescan/stable
            self.requires("glib/2.78.3@tescan/stable")
        if self.options.with_doubleconversion and not self.options.multiconfiguration:
            # tescan needs this with tescan/stable
            self.requires("double-conversion/3.3.0@tescan/stable")
        if self.options.get_safe("with_freetype", False) and not self.options.multiconfiguration:
            # tescan needs this with tescan/stable
            self.requires("freetype/2.13.2@tescan/stable")
        ...
        if self.options.get_safe("with_harfbuzz", False) and not self.options.multiconfiguration:
            # tescan needs this with tescan/stable
            self.requires("harfbuzz/8.3.0@tescan/stable")
        if self.options.get_safe("with_libjpeg", False) and not self.options.multiconfiguration:
            if self.options.with_libjpeg == "libjpeg-turbo":
                self.requires("libjpeg-turbo/[>=3.0 <3.1]")
            else:
                # tescan needs this with tescan/stable
                self.requires("libjpeg/9e@tescan/stable")
        if self.options.get_safe("with_libpng", False) and not self.options.multiconfiguration:
            # tescan needs this with tescan/stable
            self.requires("libpng/1.6.40@tescan/stable")
        if self.options.with_sqlite3 and not self.options.multiconfiguration:
            # tescan needs this with tescan/stable
            self.requires("sqlite3/[>=3.45.0 <4]@tescan/stable")
        if self.options.get_safe("with_mysql", False):
            self.requires("libmysqlclient/8.1.0")
        if self.options.with_pq:
            # tescan needs this with tescan/stable
            self.requires("libpq/15.4@tescan/stable")
        ...
        if self.options.with_brotli:
            # tescan needs this with tescan/stable
            self.requires("brotli/1.1.0@tescan/stable")
    ...
    def build_requirements(self):
        ...
        if self.options.get_safe("qtwebengine"):
            ...
            if self._settings_build.os == "Windows":
                # tescan needs this with tescan/stable
                self.tool_requires("winflexbison/2.5.25@tescan/stable")
            else:
